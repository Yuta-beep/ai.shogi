...
fn sample_skill_definitions_validate_against_builtin_registry() {
    ...
    assert_eq!(parsed.definitions.len(), 102);
}
...
fn runtime_rules_load_skill_registry_and_definitions_v2() {
    ...
    assert_eq!(rules.skill_runtime.definitions.len(), 102);
    assert_eq!(
...
#[test]
fn remaining_skill_script_hooks_use_named_hooks_and_expected_conditions() {
    for (skill_id, hook, condition_len) in [
        (101_u64, "discount_enemy_elite_to_pawn", 0_usize),
        (104_u64, "eject_enemy_piece_to_hand", 1_usize),
    ] {
        let definition = sample_skill_by_id(skill_id);
        assert_eq!(definition.classification.implementation_kind, "script_hook");
        assert_eq!(definition.trigger.group, "event_move");
    ...
    assert_eq!(
        sample_skill_by_id(104).conditions[0].type_code,
        "chance_roll"
    );
}
...
#[test]
fn song_sample_is_an_after_move_summon_piece_skill() {
    let definition = sample_skill_by_id(102);
    assert_eq!(definition.classification.implementation_kind, "primitive");
    assert_eq!(definition.trigger.group, "event_move");
    assert_eq!(definition.trigger.type_code, "after_move");
    assert_eq!(definition.conditions.len(), 2);
    ...
    assert_eq!(definition.effects[0].type_code, "summon_piece");
    assert_eq!(definition.effects[0].target.selector, "adjacent_empty");
    assert_eq!(definition.effects[0].params["summonPieceCode"], "KI");
}
...
#[test]
fn advance_sample_is_a_turn_start_self_modify_movement_skill() {
    let definition = sample_skill_by_id(105);
    assert_eq!(definition.classification.implementation_kind, "primitive");
    assert_eq!(definition.trigger.group, "event_turn");
    assert_eq!(definition.trigger.type_code, "turn_start");
    assert!(definition.conditions.is_empty());
...
#[test]
fn spec_an_downgrades_the_first_enemy_elite_piece_to_pawn() {
    let rules = sample_runtime_rules();
    let state = SearchState::from_sfen("4k4/9/2s6/9/4R4/9/6g2/9/4K4 b - 1").expect("must parse");
    let mv = EngineMove {
        from_row: Some(4),
        from_col: Some(4),
        to_row: 4,
        to_col: 5,
        piece_code: "安".to_string(),
        promote: false,
        drop_piece_code: None,
        captured_piece_code: None,
        notation: None,
    ...
    let simulated = simulate_move_with_skills(&state, &mv, &rules).expect("hook must apply");
    assert!(simulated.trace.applied_skill_ids.contains(&101));
    assert_eq!(
        simulated.trace.applied_hooks,
        vec!["discount_enemy_elite_to_pawn".to_string()]
    );
    let transformed = simulated.state.board[2 * 9 + 2].expect("silver must remain on board");
...
#[test]
fn spec_itsu_ejects_the_first_enemy_piece_to_hand() {
    let rules = sample_runtime_rules();
    let state = SearchState::from_sfen("4k4/9/2s6/9/4R4/9/9/9/4K4 b - 1").expect("must parse");
    let mv = EngineMove {
        from_row: Some(4),
        from_col: Some(4),
        to_row: 4,
        to_col: 5,
        piece_code: "逸".to_string(),
        promote: false,
        drop_piece_code: None,
        captured_piece_code: None,
        notation: None,
    ...
    let simulated = simulate_move_with_skills(&state, &mv, &rules).expect("hook must apply");
    assert!(simulated.trace.applied_skill_ids.contains(&104));
    assert_eq!(
        simulated.trace.applied_hooks,
        vec!["eject_enemy_piece_to_hand".to_string()]
    );
    assert!(simulated.state.board[2 * 9 + 2].is_none());
    assert_eq!(simulated.state.hands[1][3], 1);
    assert!(simulated.expected_value > 0.29 && simulated.expected_value < 0.31);
}
...
#[test]
fn spec_song_summons_a_gold_into_the_first_adjacent_empty_cell() {
    let rules = sample_runtime_rules();
    let state = SearchState::from_sfen("4k4/9/9/9/4R4/9/9/9/4K4 b - 1").expect("must parse");
    let mv = EngineMove {
        from_row: Some(4),
    ...
    let simulated = simulate_move_with_skills(&state, &mv, &rules).expect("skill must apply");
    assert!(simulated.trace.applied_skill_ids.contains(&102));
    assert_eq!(
        simulated.trace.applied_effect_steps,
        vec!["1:summon_piece:adjacent_empty".to_string()]
    );
    let summoned = simulated.state.board[3 * 9 + 4].expect("gold must be summoned");
    assert_eq!(summoned.side, Side::Black);
    assert_eq!(summoned.kind, PieceKind::Gold);
    assert!(simulated.expected_value > 0.19 && simulated.expected_value < 0.21);
}
...
#[test]
fn spec_advance_marks_turn_start_modify_movement() {
    let rules = sample_runtime_rules();
    let state = SearchState::from_sfen("4k4/9/9/9/4R4/9/9/9/4K4 b - 1").expect("must parse");
    let mv = EngineMove {
        from_row: Some(4),
    ...
    let simulated = simulate_move_with_skills(&state, &mv, &rules).expect("skill must apply");
    assert!(simulated.trace.applied_skill_ids.contains(&105));
    assert_eq!(
        simulated.trace.applied_effect_steps,
        vec!["1:modify_movement:self_piece".to_string()]
    );
    assert!(simulated
        ...
        .turn_start_rules
        .iter()
        .any(|rule| rule.row == 4 && rule.col == 5 && rule.rule_type == "cyclic_pattern_change"));
}
