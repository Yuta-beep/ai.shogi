...
fn execute_summon_piece(state: &mut SearchState, mv: &EngineMove, effect: &SkillEffect) -> bool {
    ...
    if source_piece.is_none() && summon_piece_code.is_none() && summon_piece_char.is_none() {
        return false;
    }
    ...
    for dr in -1..=1 {
        for dc in -1..=1 {
            ...
            } else if let Some(kind) = summon_piece_code {
                state.board[idx] = Some(Piece {
                    side: state.side_to_move.opposite(),
                    kind,
                    promoted: false,
                });
            }
...
fn execute_script_hook(state: &mut SearchState, mv: &EngineMove, hook: &str) -> bool {
    match hook {
        ...
        "discount_enemy_elite_to_pawn" => execute_discount_enemy_elite_to_pawn(state),
        "eject_enemy_piece_to_hand" => execute_eject_enemy_piece_to_hand(state),
...
fn execute_discount_enemy_elite_to_pawn(state: &mut SearchState) -> bool {
    let enemy_side = state.side_to_move;
    for idx in 0..81 {
        let Some(piece) = state.board[idx] else {
            continue;
        ...
        if !matches!(piece.kind, PieceKind::Gold | PieceKind::Silver) {
            continue;
        }
        state.board[idx] = Some(Piece {
            side: piece.side,
...
fn execute_eject_enemy_piece_to_hand(state: &mut SearchState) -> bool {
    let enemy_side = state.side_to_move;
    for idx in 0..81 {
        let Some(piece) = state.board[idx] else {
            continue;
